...
class YoutubeCard:
    def __init__(self, folder_path, movie_name:str, popup_master:tk.Widget):
        logger.debug(f'Creating YoutubeCard for {movie_name}')
    ...
    def poster_image(self):
        if not hasattr(self, '_poster_image'):
            ...
            try:
                ...
                try:
                    self._poster_image = ImageTk.PhotoImage(Image.open(alt_poster_image_loc).resize((ui_sizes.YOUTUBE_WIDTH, ui_sizes.YOUTUBE_HEIGHT)))
                    self.poster_url = alt_poster_image_loc
                except FileNotFoundError:
                    logger.warning(f'No image file for {self.movie_name}')
                    try:
                        self._poster_image = ImageTk.PhotoImage(Image.open(extract_mp4_thumbnail(self.video_path)).resize((ui_sizes.YOUTUBE_WIDTH, ui_sizes.YOUTUBE_HEIGHT)))
                        ...
                        try:
                            ...
                            self._poster_image = ImageTk.PhotoImage(Image.fromarray(snapshot).resize((ui_sizes.YOUTUBE_WIDTH, ui_sizes.YOUTUBE_HEIGHT)))
                        except Exception as e:
    ...
    def interpret_nfo(self):
        ...
        json_tree = info_cache.get_youtube_info_json(self.info_json_path)
