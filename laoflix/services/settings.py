...
SETTINGS_FILE = os.path.join(os.environ['LOCALAPPDATA'], 'laoflix', 'settings.json')
...
class Settings:
    ...
    @property
    def youtube_folders(self) -> list[str]:
        '''
        Gets the YouTube folders.

        Returns:
          list: The YouTube folders.
        '''
        if not self.get('youtube_folders'):
            self.set('youtube_folders', [])
        return self.get('youtube_folders')
    
    def add_youtube_folder(self, folder):
        '''
        Adds a YouTube folder.

        Args:
          folder (str): The folder to be added.
        '''
        youtube_folders = self.youtube_folders
        youtube_folders.append(folder)
        self.set('youtube_folders', youtube_folders)
